...
class GPR_model(tfp.distributions.GaussianProcessRegressionModel):
    ...
    def V_g(self,stock = True):
        ...
        temp2 = tf.identity(self.index_points+self.jitter**2) #Add small amount of jitter to avoid evaluating in zero
    ...
    def plot_price(self,true_price,bounds = True,confidence = 0.95,fig_name=None):
        lBound , uBound = self.bounds_price(confidence)
        return self.plot(self.price(),true_price,bounds,confidence,lBound,uBound,'Option Price',fig_name)
    ...
    def plot(self,y_pred,true_values,bounds,confidence,u_bound,l_bound,ylabel,fig_name=None):
